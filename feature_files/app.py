...
def start_price_streaming():
    ...
    def stream_prices():
        ...
        # OKX adapter state
        okx_spot_adapter = None
        okx_futures_adapter = None
        okx_loop = None  # Event loop for async OKX calls
        ...
        while price_streaming_active:
            try:
                ...
                # Fetch OKX prices - maintain persistent connection
                if spot_broker.broker_type == 'OKX' or futures_broker.broker_type == 'OKX':
                    try:
                        import asyncio
                        import os
                        from adapters.okx_adapter import OKXAdapter
                        from adapters.base import BrokerConfig as AdapterBrokerConfig

                        # Create event loop if needed
                        if okx_loop is None:
                            okx_loop = asyncio.new_event_loop()

                        # Initialize OKX spot adapter if needed
                        if spot_broker.broker_type == 'OKX' and okx_spot_adapter is None:
                            config = AdapterBrokerConfig(
                                broker_id=spot_broker.broker_id,
                                name=spot_broker.name,
                                role='SPOT',
                                backend_type='OKX',
                                symbol=spot_broker.symbol,
                                okx_api_key=spot_broker.okx_api_key or os.environ.get('OKX_API_KEY', ''),
                                okx_api_secret=spot_broker.okx_api_secret or os.environ.get('OKX_API_SECRET', ''),
                                okx_passphrase=spot_broker.okx_passphrase or os.environ.get('OKX_PASSPHRASE', ''),
                                okx_simulated=getattr(spot_broker, 'okx_simulated', True),
                                okx_account_type=getattr(spot_broker, 'okx_account_type', 'spot'),
                            )
                            okx_spot_adapter = OKXAdapter(config)
                            connected = okx_loop.run_until_complete(okx_spot_adapter.connect())
                            if connected:
                                logger.info(f"[PRICES] OKX spot adapter connected (persistent) - {spot_broker.symbol}")
                            else:
                                logger.warning("[PRICES] OKX spot adapter connection failed, will retry")
                                okx_spot_adapter = None
                                time.sleep(2)
                                continue

                        # Initialize OKX futures adapter if needed
                        if futures_broker.broker_type == 'OKX' and okx_futures_adapter is None:
                            config = AdapterBrokerConfig(
                                broker_id=futures_broker.broker_id,
                                name=futures_broker.name,
                                role='FUTURES',
                                backend_type='OKX',
                                symbol=futures_broker.symbol,
                                okx_api_key=futures_broker.okx_api_key or os.environ.get('OKX_API_KEY', ''),
                                okx_api_secret=futures_broker.okx_api_secret or os.environ.get('OKX_API_SECRET', ''),
                                okx_passphrase=futures_broker.okx_passphrase or os.environ.get('OKX_PASSPHRASE', ''),
                                okx_simulated=getattr(futures_broker, 'okx_simulated', True),
                                okx_account_type=getattr(futures_broker, 'okx_account_type', 'swap'),
                            )
                            okx_futures_adapter = OKXAdapter(config)
                            connected = okx_loop.run_until_complete(okx_futures_adapter.connect())
                            if connected:
                                logger.info(f"[PRICES] OKX futures adapter connected (persistent) - {futures_broker.symbol}")
                            else:
                                logger.warning("[PRICES] OKX futures adapter connection failed, will retry")
                                okx_futures_adapter = None
                                time.sleep(2)
                                continue

                        # Get spot price from OKX
                        if spot_broker.broker_type == 'OKX' and okx_spot_adapter:
                            tick = okx_loop.run_until_complete(okx_spot_adapter.get_tick(spot_broker.symbol))
                            if tick:
                                spot_bid = tick.bid
                                spot_ask = tick.ask
                            else:
                                logger.warning(f"[PRICES] OKX spot tick failed for {spot_broker.symbol}")
                                okx_spot_adapter = None  # Will reconnect
                                continue

                        # Get futures price from OKX
                        if futures_broker.broker_type == 'OKX' and okx_futures_adapter:
                            tick = okx_loop.run_until_complete(okx_futures_adapter.get_tick(futures_broker.symbol))
                            if tick:
                                futures_bid = tick.bid
                                futures_ask = tick.ask
                            else:
                                logger.warning(f"[PRICES] OKX futures tick failed for {futures_broker.symbol}")
                                okx_futures_adapter = None  # Will reconnect
                                continue

                        # Update cached leverage from OKX (default to 10x for crypto)
                        if not cached_leverage or cached_leverage == 100:
                            cached_leverage = 10  # Default leverage for OKX

                    except ImportError as e:
                        logger.error(f"[PRICES] OKX adapter import failed: {e}")
                    except Exception as e:
                        logger.error(f"[PRICES] OKX price fetch error: {e}")
                        okx_spot_adapter = None
                        okx_futures_adapter = None
